...
public:
	...
	EXECUTOR_REQUIRE(detail::windows_registry)
	bool fix_server_emulator_save(std::string_view target_field, std::string_view custom_save)
	{
		select(target_field);
	...
	EXECUTOR_REQUIRE(detail::windows_registry)
	bool fix_offline_save(std::string_view target_field, std::string_view custom_save)
	{
		select(target_field);
